...
class SLP(Algorithm):
    ...
    def load_data(self, data_path):
        ...
        if 'parquet' in data_path or 'use_parquet' in self.options and self.options['use_parquet']:
            ...
            rdd_vals_only = self.sc.newAPIHadoopFile(data_path, 'com.cotdp.hadoop.ZipFileInputFormat',
                                      ...
                                      'org.apache.hadoop.io.Text').map(lambda a_b: a_b[1])
    ...
    def train(self, all_tweets, predictions_curve=None):
        ...
        full_edge_list = self.sqlCtx.sql('select user.id_str, entities.user_mentions from %s where size(entities.user_mentions) > 0'%\
            ...
            .flatMap(lambda a_b4: SLP.filter_non_bidirectional(a_b4[1])).coalesce(300)
        ...
        filter_function = lambda a_b9: a_b9[0][-1] not in options.value['hold_out']
        ...
        if predictions_curve is None:
            ...
            user_location_only = original_user_locations.map(lambda a_b1: (a_b1[0], a_b1[1][0]))
            adj_list_w_locations = full_edge_list.join(user_location_only).map(lambda a_b2: (a_b2[1][0], (a_b2[1][1],a_b2[0]))).groupByKey()
            ...
            std_mults = network_info.map\
                (lambda id_str_lat0_lat1_disp_mean_dis_std_dev : (haversine(id_str_lat0_lat1_disp_mean_dis_std_dev[1][0][1], id_str_lat0_lat1_disp_mean_dis_std_dev[1][0][0], id_str_lat0_lat1_disp_mean_dis_std_dev[1][0][1], id_str_lat0_lat1_disp_mean_dis_std_dev[1][0][0]) - id_str_lat0_lat1_disp_mean_dis_std_dev[1][1])/id_str_lat0_lat1_disp_mean_dis_std_dev[1][3])
        ...
        filtered_edge_list = full_edge_list.keyBy(lambda a_b5: a_b5[1]).leftOuterJoin(updated_locations)\
                .flatMap(lambda a_b6: [a_b6[1][0]] if a_b6[1][1] is None else [])
    ...
    def do_iteration(self, pull_to_local_ctx=False):
        ...
        location_only = self.updated_locations.map(lambda a_b7: (a_b7[0], a_b7[1][0]))
        adj_list_w_locations = self.filtered_edge_list.join(location_only).map(lambda a_b8: (a_b8[1][0], (a_b8[1][1],a_b8[0]))).groupByKey()
    ...
    def test(self, all_tweets, skip_load=False):
        ...
        filter_function = lambda a_b10: a_b10[0][-1] in options.value['hold_out']
        original_user_locations = original_user_locations.filter(filter_function)
        number_locations = original_user_locations.count()

        found_locations = original_user_locations.join(self.updated_locations.map(lambda a_b3: (a_b3[0], a_b3[1][0])))
